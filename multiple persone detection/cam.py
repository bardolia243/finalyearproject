...
while 1:
   ...
   for (x,y,w,h) in faces:
        ...
        cv2.rectangle(img,(x,y),(x+w,y+h),(41,36,33),2)
        ...
        x = x + w
        print('x=',x)
        cv2.putText(img,'Y=' + str(y) , (img.shape[1] - 600, img.shape[0] - 50) , cv2.FONT_HERSHEY_SIMPLEX,0.7, (128,0,0), 2)
        cv2.putText(img, 'X=' +  str(x) ,(img.shape[1] - 600, img.shape[0] - 100), cv2.FONT_HERSHEY_SIMPLEX,0.7, (128,0,0), 2)
        ...
        cv2.putText(img,'Distance =' + str(Distance) ,(img.shape[1] - 200, img.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX,0.7, (238, 224, 229), 3)
        cv2.putText(img,'person count =' + str(count) ,(img.shape[1] - 200, img.shape[0] - 100), cv2.FONT_HERSHEY_SIMPLEX,0.7, (238, 224, 229), 3)
        D = Distance
        if D<10:
        ...
        A = s
        #print(A)
        cv2.putText(img, str(A) ,(img.shape[1] - 630, img.shape[0] - 400), cv2.FONT_HERSHEY_SIMPLEX,0.7, (0,0,0), 2)
        import datetime
        now = datetime.datetime.now()
        B=now.strftime("%Y-%m-%d %H:%M:%S")
        #print ("Current date and time : ")
        #print (B)
        cv2.putText(img, str(B) ,(img.shape[1] - 630, img.shape[0] - 450), cv2.FONT_HERSHEY_SIMPLEX,1, (0,100,0), 2)
